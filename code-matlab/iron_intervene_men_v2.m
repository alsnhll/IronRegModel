...
function iron_intervene_men_v2
...
Int = [0, 5];               % Daily intake supplement [mg/day]
...
for j = 1:length(hb_int)
    for k=1:length(Int)
        for i=1:length(red_men)
            ...
            [results{j, k, i}.T, results{j, k, i}.Y] = ode45(@(t, x) ironsolve(t, x, Int_ss(j)+(t<=90)*Int(k)/1000, e1*(1-red_men(i)/100), e2, d, h0, conv), [0 Tend]*30, [OBI0(j); hb_int(j)/conv]);
    ...
    for k=1:length(Int)
        for i=1:length(red_men)
            for l=1:length(hev_per)
                ...
                [results_2{j, k, i, l}.T, results_2{j, k, i, l}.Y] = ode45(@(t, x) ironsolve(t, x, Int_ss(j)+(t<=90)*Int(k)/1000, hev_per(l)*e1*(1-red_men(i)/100), e2, d, h0, conv), [0 Tend]*30, [OBI0(j); hb_int(j)/conv]);
            end
        end
    end
end
